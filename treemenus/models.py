...
class MenuItem(models.Model):
    ...
    named_url = models.CharField(ugettext_lazy('named URL'),
                                 max_length=200, blank=True)
    level = models.IntegerField(ugettext_lazy('level'),
                                default=0, editable=False)
    rank = models.IntegerField(ugettext_lazy('rank'),
                               default=0, editable=False)
    ...
    def siblings(self):
        if not self.parent:
            ...
            if not self.pk:
                # If menu item not yet been saved in DB
                # (i.e does not have a pk yet)
